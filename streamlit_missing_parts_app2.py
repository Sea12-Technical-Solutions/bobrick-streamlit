...
def _call_openai_vision_compare(upload_data_url: str, context_data_url: str, model: str) -> dict:
    ...
    system_prompt = (
        ...
        "Do not compare the colors of objects - focus on the presence or absence of physical parts only."
    )
    ...
    user_instructions = (
        ...
        "Missing parts are likely to be the hose on the right side or a tag. "
        "Do not compare colors - only check for the presence or absence of physical parts/components. "
...
def main() -> None:
    st.set_page_config(page_title="Autosoap Assembly Verification", page_icon="🔍", layout="centered")
    st.title("🔍 Autosoap Assembly Verification")
    st.caption(
    ...
    default_context_path = Path(__file__).resolve().parent / "autosoapref.JPG"
